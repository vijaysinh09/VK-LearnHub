  ...
  try {
    ...
    if (type === "website") {
      messages.push({
        role: "system",
        content:
          "You are an AI assistant strictly for VK LearnHub, an online education platform. Only answer questions related to VK LearnHub, its courses, enrollments, or platform features. If a user asks anything outside of this scope, politely decline and state you are only here to help with VK LearnHub.",
      });
    }
...
app.post("/courses", async (req, res) => {
  try {
    ...
    const sql =
      "insert into courses(title,description,duration,price,instructor_id)values(?,?,?,?,?)";
...
app.put("/courses/:id", async (req, res) => {
  try {
    ...
    const { title, description, duration, price, instructor_id } = req.body;
    const sql =
      "update courses set title=?,description=?,duration=?,price=?,instructor_id=? where id=?";
    const [result] = await db.query(sql, [
      title,
      description,
      duration,
...
app.get("/coursedetails/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const sql =
      "select courses.title AS CourseName,courses.description AS Information,courses.duration AS Period,courses.price AS Amount,users.name AS Instructor from courses join users on courses.instructor_id=users.id where courses.id=?";
    const [result] = await db.query(sql, [id]);
...
app.get("/cards", async (req, res) => {
  try {
    const sql =
      "select(select count(*)from users) AS TotalUsers,(select count(*)from courses) AS TotalCourses,(select count(*)from users WHERE role='instructor') AS TotalInstructors,(select count(*)from users WHERE role='student') AS TotalStudents";
    const [result] = await db.query(sql);
    res.json(result[0]);
  } catch (err) {
    res.status(500).json({
...
app.post("/courses/bulkupload", async (req, res) => {
  try {
    ...
    if (!Array.isArray(courses) || courses.length === 0) {
      return res
        .status(400)
        .json({ message: "No courses provided for bulk upload" });
    }
