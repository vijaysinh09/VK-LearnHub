...
function InstructorDashboard() {
  ...
  const fetchStats = () =>
    axios
      .get("https://vk-learnhub-1.onrender.com/instructor/stats")
      .then((r) => setStats(r.data))
      .catch(console.error);
  const fetchStudents = () =>
    axios
      .get("https://vk-learnhub-1.onrender.com/users")
      .then((r) => setStudents(r.data))
      .catch(console.error);
  const fetchCourses = () =>
    axios
      .get("https://vk-learnhub-1.onrender.com/courses")
      .then((r) => setCourses(r.data))
      .catch(console.error);
  const fetchEnrollments = () =>
    axios
      .get("https://vk-learnhub-1.onrender.com/instructor/enrollments")
  ...
  const updateUser = async () => {
    try {
      ...
      await axios.put(
        `https://vk-learnhub-1.onrender.com/instructor/users/${id}`,
        { name, email, role },
      );
  ...
  const saveCourse = async () => {
    try {
      if (modal.type === "addCourse") {
        ...
        await axios.put(
          `https://vk-learnhub-1.onrender.com/courses/${courseForm.id}`,
          courseForm,
        );
  ...
  const deleteEnrollment = async (id) => {
    ...
    try {
      await axios.delete(
        `https://vk-learnhub-1.onrender.com/enrollments/${id}`,
      );
      fetchEnrollments();
      fetchStats();
    } catch (err) {
  ...
  const updateEnrollmentStatus = async (id, status) => {
    try {
      await axios.put(
        `https://vk-learnhub-1.onrender.com/instructor/enrollments/${id}`,
        { status },
      );
      fetchEnrollments();
    } catch (err) {
          ...
          <>
            <div className="ad-page-header">
              <div>
                ...
                <p className="ad-page-sub">
                  Welcome back, {loguser.name}. Here's your platform overview.
                </p>
              </div>
            </div>
            ...
            <div className="ad-grid">
              <div className="ad-widget">
                ...
                {enrollments.length === 0 ? (
                  <div className="ad-empty">
                    <div className="ad-empty-icon">📋</div>No enrollments yet.
                  </div>
                ) : (
                      ...
                      <div key={e.enrollmentId} className="ad-activity-item">
                        ...
                        <div>
                          <div className="ad-activity-text">
                            <strong>{e.studentName}</strong> enrolled in{" "}
              ...
              <div className="ad-widget">
                ...
                {courses.length === 0 ? (
                  <div className="ad-empty">
                    <div className="ad-empty-icon">📦</div>No courses yet.
                  </div>
                ) : (
                      ...
                      <tbody>
                        {courses.slice(0, 5).map((c) => (
                          <tr key={c.id}>
                            ...
                            <td>{c.duration}</td>
                            <td style={{ fontWeight: 700, color: "#7e22ce" }}>
                              ₹{c.price}
                            </td>
                          </tr>
                        ))}
          ...
          <>
            <div className="ad-page-header">
              <div>
                ...
                <p className="ad-page-sub">
                  {students.length} registered users in the system.
                </p>
              </div>
              <input
                className="ad-search"
            ...
            <div className="ad-widget">
              {filteredStudents.length === 0 ? (
                <div className="ad-empty">
                  <div className="ad-empty-icon">👤</div>No users found.
                </div>
              ) : (
                <div className="ad-table-wrap">
                  <table className="ad-table">
                    ...
                    <tbody>
                      {filteredStudents.map((s, i) => (
                        <tr key={s.id}>
                          ...
                          <td>
                            <span
                              className={`badge ${s.role === "instructor" ? "badge-purple" : s.role === "instructor" ? "badge-blue" : "badge-green"}`}
                            >
                              {s.role}
                            </span>
                          </td>
          ...
          <>
            ...
            <div className="ad-widget">
              {filteredCourses.length === 0 ? (
                <div className="ad-empty">
                  <div className="ad-empty-icon">📦</div>No courses found.
                </div>
              ) : (
                <div className="ad-table-wrap">
                  <table className="ad-table">
          ...
          <>
            ...
            <div className="ad-widget">
              {filteredEnrollments.length === 0 ? (
                <div className="ad-empty">
                  <div className="ad-empty-icon">🎯</div>No enrollments found.
                </div>
              ) : (
                <div className="ad-table-wrap">
                  <table className="ad-table">
                  ...
                  {[
                    {
                      ...
                      val: stats.activeEnrollments || 0,
                      total: stats.totalEnrollments || 1,
                      color: "#10b981",
                    },
                    {
                      label: "Completed Enrollments",
                      val: stats.completedEnrollments || 0,
                      total: stats.totalEnrollments || 1,
                      color: "#3b82f6",
                    },
                  ].map((r) => (
                    <div key={r.label}>
                      ...
                      >
                        <span>{r.label}</span>
                        <strong>
                          {r.val} ({Math.round((r.val / r.total) * 100) || 0}%)
                        </strong>
                      </div>
                      <div
                        style={{
                      ...
                      >
                        <div
                          style={{
                            width: `${Math.round((r.val / r.total) * 100) || 0}%`,
                            height: "100%",
                            borderRadius: 999,
                            background: r.color,
                ...
                {enrollments.length === 0 ? (
                  ...
                  (() => {
                    ...
                    enrollments.forEach((e) => {
                      counts[e.courseTitle] = (counts[e.courseTitle] || 0) + 1;
                    });
            ...
            <div className="ad-modal-field">
              ...
              <select
                ...
                onChange={(e) =>
                  setModal({
                    ...modal,
                    data: { ...modal.data, role: e.target.value },
                  })
                }
              >
            ...
            {[
              {
                ...
                name: "title",
                type: "text",
                ph: "e.g. Web Development",
              },
              {
                label: "Description",
                name: "description",
                type: "text",
                ph: "Short course description",
              },
              {
                label: "Duration",
                name: "duration",
              ...
              <div className="ad-modal-field" key={f.name}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setCourseForm({ ...courseForm, [f.name]: e.target.value })
                  }
                />
              </div>
            ))}
