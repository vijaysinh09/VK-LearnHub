...
function Coursedetails() {
  ...
  if (loading) return (
    <div className="cd-page">
      <div className="cd-card" style={{textAlign:'center',color:'#9ca3af'}}>Loading course details…</div>
    </div>
  );
    ...
    <div className="cd-page">
      <div className="cd-card">
        <div className="cd-back">
          <Link to="/student-dashboard" state={{ tab: 'courses' }} className="btn btn-outline btn-sm">← Back to Courses</Link>
        </div>

        <div className="cd-emoji">📘</div>
        <h1 className="cd-title">{course.CourseName || "Course Details"}</h1>
        <p className="cd-subtitle">{course.Information || "Detailed course information."}</p>
        ...
        <div className="cd-note">
          📌 <strong>Note:</strong> If any details are missing, please check back later or contact support.
        </div>

        <div className="cd-actions" style={{ display: 'flex', justifyContent: 'center' }}>
          <button className="btn btn-primary btn-lg" onClick={handleEnroll} style={{ paddingLeft: '40px', paddingRight: '40px' }}>
           Pay Now
          </button>
        </div>
      </div>
