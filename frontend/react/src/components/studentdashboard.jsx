...
function StudentDashboard() {
    ...
    website: [
      {
        role: "bot",
        content:
          "Hi! I am the VK LearnHub Assistant. How can I help you with our platform today?",
      },
    ],
    general: [
      {
        role: "bot",
        content: "Hi! I am your General AI Assistant. Ask me anything!",
      },
    ],
  });
  ...
  const progressPct = myCourses.length
    ? Math.round((doneCount / myCourses.length) * 100)
    : 0;

  const fetchCards = () =>
    axios
      .get("https://vk-learnhub-1.onrender.com/cards")
      .then((r) => setCards(r.data))
      .catch(console.error);
  const fetchMyCourses = () =>
    axios
      .get(`https://vk-learnhub-1.onrender.com/enrollments/${loguser?.id}`)
      .then((r) => setMyCourses(r.data))
      .catch(console.error);
  const fetchAllCourses = (page = 1) => {
    axios
      .get(
        `https://vk-learnhub-1.onrender.com/courses_pagination?page=${page}&limit=8`,
      )
      .then((r) => {
  ...
  const handleUnenroll = async (enrollmentId) => {
    ...
    try {
      await axios.delete(
        `https://vk-learnhub-1.onrender.com/enrollments/${enrollmentId}`,
      );
      fetchMyCourses();
      fetchAllCourses();
    } catch (err) {
  ...
  const handleSendMessage = async (e, currentMode) => {
    ...
    try {
      const res = await axios.post("https://vk-learnhub-1.onrender.com/chat", {
        message: userMessage,
        type: currentMode,
      });
      setChatMessages((prev) => ({
        ...prev,
        [currentMode]: [...newMsgs, { role: "bot", content: res.data.reply }],
      }));
    } catch (err) {
      setChatMessages((prev) => ({
        ...prev,
            ...
            <div className="sd-grid">
              ...
              <div className="sd-widget">
                <div className="sd-widget-header">
                  ...
                  <button
                    className="sd-widget-link"
                    style={{
                      background: "none",
                      border: "none",
                      cursor: "pointer",
                    }}
                    onClick={() => setActivePage("enrolled")}
                  >
                    View All →
                  </button>
                </div>
                    ...
                    >
                      ...
                      {c.status === "completed" ? (
                        ...
                        <button
                          ...
                          onClick={() =>
                            navigate(`/course-learning/${c.courseId}`, {
                              state: { course: c },
                            })
                          }
                        >
              ...
              <div className="sd-widget">
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  >
                    ...
                    <span style={{ fontWeight: 700, color: "#4C2A78" }}>
                      {progressPct}%
                    </span>
                  </div>
                ...
                <div className="sd-pay-row">
                  <span className="sd-pay-label">Total Enrolled</span>
                  <span className="sd-pay-value">{myCourses.length}</span>
                </div>
                <div className="sd-pay-row">
                  <span className="sd-pay-label">Active Courses</span>
                  <span className="sd-pay-value">{activeCount}</span>
                </div>
                <div className="sd-pay-row">
                  <span className="sd-pay-label">Completed</span>
                  <span className="sd-pay-value">{doneCount}</span>
                </div>
                <div className="sd-pay-row sd-pay-total">
                  <span className="sd-pay-label">Total Invested</span>
                  <span className="sd-pay-value">₹{totalSpent}</span>
                </div>
              </div>
              ...
              <div className="sd-widget">
                ...
                {availCourses.length === 0 ? (
                  <div className="sd-empty">
                    🎉 You've explored all available courses!
                  </div>
                ) : (
                  ...
                  <div className="sd-notif-list">
                    {myCourses.slice(0, 5).map((c) => (
                      <div key={c.enrollmentId} className="sd-notif-item">
                        ...
                        <span>
                          Enrolled in <strong>{c.title}</strong> — {c.status}
                        </span>
                      </div>
                    ))}
                  </div>
                ...
                >
                  {[
                    ...
                    <div
                      key={s.label}
                      style={{
                        background: "linear-gradient(135deg,#FAF7FF,#EDE6F5)",
                        borderRadius: 14,
                        padding: "20px 14px",
                        textAlign: "center",
                    ...
                    <tbody>
                      {myCourses.map((c, i) => (
                        <tr key={c.enrollmentId}>
                          ...
                          <td>{c.duration}</td>
                          <td style={{ fontWeight: 700, color: "#4C2A78" }}>
                            ₹{c.price}
                          </td>
                          <td>
                            {c.enrolled_at
                          ...
                          <td>
                            <span
                              className={`badge ${c.status === "completed" ? "badge-blue" : "badge-green"}`}
                            >
                              {c.status}
                            </span>
                          </td>
          ...
          <>
            <div className="sd-page-header">
              <div>
                ...
                <p className="sd-page-sub">
                  Record of all your course investments.
                </p>
              </div>
            </div>
          ...
          <>
            <div className="sd-page-header">
              <div>
                ...
                <p className="sd-page-sub">
                  Your account information and learning summary.
                </p>
              </div>
            </div>
            ...
            <div className="sd-grid">
              <div className="sd-widget">
                ...
                <div className="sd-pay-row">
                  ...
                  <span className="sd-pay-value">Student</span>
                </div>
                <div className="sd-pay-row">
                  <span className="sd-pay-label">User ID</span>
                  <span className="sd-pay-value">#{loguser.id}</span>
                </div>
              </div>
              ...
              <div className="sd-widget">
                ...
                <div className="sd-pay-row">
                  <span className="sd-pay-label">Courses Enrolled</span>
                  <span className="sd-pay-value">{myCourses.length}</span>
                </div>
                <div className="sd-pay-row">
                  <span className="sd-pay-label">Active Courses</span>
                ...
                <div className="sd-pay-row">
                  <span className="sd-pay-label">Completion Rate</span>
                  <span className="sd-pay-value">{progressPct}%</span>
                </div>
                <div className="sd-pay-row sd-pay-total">
                  <span className="sd-pay-label">Total Invested</span>
                  <span className="sd-pay-value">₹{totalSpent}</span>
      ...
      <div className="sd-chatbot-container">
        ...
        {openBot && (
          <div className="sd-chat-window">
            <div
              className="sd-chat-header"
              style={{
                background: openBot === "website" ? "#7e22ce" : "#10b981",
              }}
            >
              <span style={{ fontWeight: 600 }}>
                {openBot === "website"
                  ? "🎓 VK LearnHub Assistant"
            ...
            <div className="sd-chat-body">
              {chatMessages[openBot].map((msg, i) => (
                <div
                  key={i}
                  className={`sd-chat-msg ${msg.role}`}
                  style={
                    msg.role === "user"
                      ? {
                          background:
                            openBot === "website" ? "#7e22ce" : "#10b981",
                          color: "#fff",
              ...
              {chatLoading && (
                <div className="sd-chat-msg bot">Thinking...</div>
              )}
            </div>
            <form
              className="sd-chat-input-area"
              onSubmit={(e) => handleSendMessage(e, openBot)}
            >
              <input
                type="text"
        ...
        {!openBot && (
          <div className="sd-chat-buttons">
            <div
              className="sd-chat-btn-wrapper tooltip-left"
              data-tooltip="General AI (Ask me anything)"
            >
              <button
                className="sd-chat-toggle general"
                onClick={() => setOpenBot("general")}
              >
                🌐
              </button>
            </div>
            <div
              className="sd-chat-btn-wrapper tooltip-left"
              data-tooltip="VK LearnHub Assistant (Platform Help)"
            >
              <button
                className="sd-chat-toggle website"
                onClick={() => setOpenBot("website")}
              >
                🎓
              </button>
            </div>
